...
public class LeakyBucketTest {
    ...
    void shouldDrainQueueBasedOnElapsedTime() {
        ...
        // Simulate: queue had 10 items, 2 seconds passed, 10 drained, 1 added = 1 in
        // queue
        when(template.execute(
                any(RedisScript.class),
                anyList(),
    ...
    void shouldHandlePartialDrain() {
        ...
        // Simulate: queue had 10, 1.5 seconds passed, 4.5 drained, 1 added = 6.5 ≈ 6 or
        // 7
        when(template.execute(
                any(RedisScript.class),
                anyList(),
    ...
    void shouldHandleVeryLargeCapacity() {
        ...
        verify(template)
                .execute(
                        any(RedisScript.class),
                        eq(List.of(key)),
                        eq(String.valueOf(Integer.MAX_VALUE)),
                        anyString(),
                        anyString(),
                        anyString());
    }
    ...
    void shouldHandleConcurrentRequestsCorrectly() throws InterruptedException {
        ...
        assertTrue(allowedCount.get() <= 20, "Should not allow more than capacity");
    }
