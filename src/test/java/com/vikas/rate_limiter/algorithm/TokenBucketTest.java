...
import com.vikas.rate_limiter.utils.RateLimiterProperties.Redis;
...
public class TokenBucketTest {
    ...
    void setUp() {
        ...
        try {
            java.lang.reflect.Field scriptField =
                    TokenBucketRateLimitAlgorithm.class.getDeclaredField("script");
        ...
        when(template.execute(any(RedisScript.class), anyList(), anyString(),
                anyString(),
        ...
        // When: Request is made
             ...
             verify(template).execute(any(RedisScript.class), eq(List.of(key)), eq("10"),
             eq("2"),
             anyString(), eq("1"), eq("600000"));
             }
        ...
        // Then: Request should be rejected
            ...
            @Test
             ...
             RateLimiterProperties.Fallback fallback = new RateLimiterProperties.Fallback();
             Map<String, Object> params = new HashMap<>();
             params.put("capacity", 20);
             params.put("refillRate", 5);
         ...
         assertEquals(1L, result.get(0), "Request should be allowed with default config");
             verify(template).execute(any(RedisScript.class), eq(List.of(key)), eq("20"),
             eq("5"),
             anyString(), eq("1"), eq("600000"));
             }
    ...
    void shouldHandleZeroRefillRate() {
        ...
        verify(template).execute(any(RedisScript.class), eq(List.of(key)), eq("5"),
                eq("0"),
                anyString(), eq("1"), eq("600000"));
    }
    ...
    void shouldHandleVeryHighRefillRate() {
        ...
        assertEquals(1L, result.get(0), "Request should be allowed");
        verify(template).execute(any(RedisScript.class), eq(List.of(key)), eq("100"),
                eq("10000"), anyString(), eq("1"), eq("600000"));
    }
    ...
    void shouldHandleConcurrentRequestsCorrectly() throws InterruptedException {
        // Given: Multiple threads making concurrent requests
            ...
            @Test
             ...
             try {
                        var method = TokenBucketRateLimitAlgorithm.class.getMethod("acceptRequest",
                                                String.class);
            ...
            @Test
             ...
             when(template.execute(any(RedisScript.class), eq(List.of(key1)), anyString(),
             anyString(), anyString(), anyString(), anyString()))
             .thenReturn(List.of(1L, 9L));
             when(template.execute(any(RedisScript.class), eq(List.of(key2)), anyString(),
             anyString(), anyString(), anyString(), anyString()))
             .thenReturn(List.of(1L, 19L));
             ...
             verify(template).execute(any(RedisScript.class), eq(List.of(key1)), eq("10"),
             eq("2"),
             anyString(), eq("1"), eq("600000"));
             verify(template).execute(any(RedisScript.class), eq(List.of(key2)), eq("20"),
             eq("5"),
             anyString(), eq("1"), eq("600000"));
             }
            ...
            @Test
             ...
             RateLimiterProperties.Fallback fallback = new RateLimiterProperties.Fallback();
             Map<String, Object> params = new HashMap<>();
             params.put("capacity", 10);
             params.put("refillRate", 2);
             fallback.setParameters(params);
             when(props.getFallback()).thenReturn(fallback);
