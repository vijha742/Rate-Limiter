...
public class SlidingWindowTest {
	...
	void shouldAllowRequestsWithinSlidingWindowLimit() {
		...
		SlidingWindowRateLimitAlgorithm algorithm = new SlidingWindowRateLimitAlgorithm(clock, 5, 10);
	...
	void shouldRejectRequestsExceedingSlidingWindowLimit() {
		TestClock clock = new TestClock(Instant.now());
		SlidingWindowRateLimitAlgorithm algorithm = new SlidingWindowRateLimitAlgorithm(clock, 3, 10);

		assertTrue(algorithm.acceptRequest());
		assertTrue(algorithm.acceptRequest());
		assertTrue(algorithm.acceptRequest());
		assertFalse(algorithm.acceptRequest(), "Should reject request exceeding limit");
	...
	void expiredRequestsShouldBeEvictedFromWindow() {
		TestClock clock = new TestClock(Instant.now());
		SlidingWindowRateLimitAlgorithm algorithm = new SlidingWindowRateLimitAlgorithm(clock, 3, 5);
		...
		assertTrue(algorithm.acceptRequest(), "Should accept after old requests expire");

		assertTrue(algorithm.getReqStorage().size() < initialSize, "Old entries should be evicted");
	}
	...
	void requestCountShouldDecreaseAsWindowSlides() {
		...
		assertTrue(algorithm.acceptRequest(), "Oldest request should have slid out of window");
	}
	...
	void shouldNotCountExpiredRequests() {
		TestClock clock = new TestClock(Instant.now());
		SlidingWindowRateLimitAlgorithm algorithm = new SlidingWindowRateLimitAlgorithm(clock, 2, 5);
	...
	void concurrentRequestsShouldNotOverCount() throws InterruptedException {
		TestClock clock = new TestClock(Instant.now());
		SlidingWindowRateLimitAlgorithm algorithm = new SlidingWindowRateLimitAlgorithm(clock, 10, 10);
		...
		assertEquals(
				10, acceptedCount.get(), "Should accept exactly maxRequests under concurrent load");
	}

	@Test
	void concurrentEvictionAndAddShouldBeThreadSafe() throws InterruptedException {
		TestClock clock = new TestClock(Instant.now());
		SlidingWindowRateLimitAlgorithm algorithm = new SlidingWindowRateLimitAlgorithm(clock, 20, 5);
		...
		assertEquals(20, acceptedCount.get(), "Concurrent eviction and add should be thread-safe");
	}

	@Test
	void shouldNotGrowWindowUnboundedUnderHighLoad() {
		TestClock clock = new TestClock(Instant.now());
		SlidingWindowRateLimitAlgorithm algorithm = new SlidingWindowRateLimitAlgorithm(clock, 100, 10);
	...
	void shouldHandleRequestsAtSameTimestamp() {
		...
		for (int i = 0; i < 5; i++) {
			assertTrue(
					algorithm.acceptRequest(), "Should handle multiple requests at same timestamp");
		}
	...
	void shouldAccuratelyTrackMultipleRequestsPerTimestamp() {
		TestClock clock = new TestClock(Instant.now());
		SlidingWindowRateLimitAlgorithm algorithm = new SlidingWindowRateLimitAlgorithm(clock, 10, 5);

		assertTrue(algorithm.acceptRequest());
		assertTrue(algorithm.acceptRequest());
		assertTrue(algorithm.acceptRequest());

		assertEquals(3, algorithm.getReqStorage().size(), "Should track each request separately");
	}
}
