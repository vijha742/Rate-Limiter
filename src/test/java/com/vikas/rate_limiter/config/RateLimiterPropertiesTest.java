...
class RateLimiterPropertiesTest {
    ...
    void testFallbackAlgorithm() {
        ...
        assertEquals(
                ...
                properties.getFallback().getAlgorithm(),
                "Fallback algorithm should be TOKEN_BUCKET");
    }

    @Test
    void testLimitsConfiguration() {
        assertNotNull(properties.getFallback(), "Limits should not be null");
        // assertEquals(10, properties.getFallback().getDefaultCapacity(),
    ...
    void testRedisConfiguration() {
        ...
        assertEquals(600000L, properties.getRedis().getTtl(), "Redis TTL should be 600000");
    }

    @Test
    void testSecurityConfiguration() {
        assertNotNull(properties.getSecurity(), "Security config should not be null");
        assertTrue(properties.getSecurity().isEnabled(), "Security should be enabled");
        assertTrue(properties.getSecurity().isLogSuspicious(), "Log suspicious should be enabled");
        assertNotNull(
                properties.getSecurity().getTrustedProxies(), "Trusted proxies should not be null");
        assertTrue(
                properties.getSecurity().getTrustedProxies().contains("127.0.0.1"),
    ...
    void testEndpointsConfiguration() {
        ...
        // assertEquals(
        // "/api/**", properties.getEndpoints().getPath(), "Endpoint path should be
        // /api/**");
    }
}
