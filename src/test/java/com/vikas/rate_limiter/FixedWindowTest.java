...
public class FixedWindowTest {
	...
	void shouldAllowRequestsUpToLimitWithinWindow() {
		...
		FixedCounterRateLimitAlgorithm algorithm = new FixedCounterRateLimitAlgorithm(5, 10, clock);
	...
	void shouldRejectRequestsExceedingWindowLimit() {
		TestClock clock = new TestClock(Instant.now());
		FixedCounterRateLimitAlgorithm algorithm = new FixedCounterRateLimitAlgorithm(3, 10, clock);
	...
	void shouldResetCounterWhenWindowExpires() {
		TestClock clock = new TestClock(Instant.now());
		FixedCounterRateLimitAlgorithm algorithm = new FixedCounterRateLimitAlgorithm(3, 5, clock);
	...
	void requestAtWindowBoundaryShouldBelongToNewWindow() {
		TestClock clock = new TestClock(Instant.now());
		FixedCounterRateLimitAlgorithm algorithm = new FixedCounterRateLimitAlgorithm(2, 5, clock);
		...
		assertTrue(algorithm.acceptRequest(), "Request at window boundary should be in new window");
	}

	@Test
	void shouldNotExceedLimitUnderConcurrentRequests() throws InterruptedException {
		TestClock clock = new TestClock(Instant.now());
		FixedCounterRateLimitAlgorithm algorithm = new FixedCounterRateLimitAlgorithm(10, 10, clock);
		...
		assertEquals(10, acceptedCount.get(), "Should accept exactly maxRequests under concurrent load");
	}

	@Test
	void concurrentIncrementsShouldBeAtomic() throws InterruptedException {
		TestClock clock = new TestClock(Instant.now());
		FixedCounterRateLimitAlgorithm algorithm = new FixedCounterRateLimitAlgorithm(100, 10, clock);
		...
		assertEquals(100, algorithm.getCounter(), "Counter should match accepted count");
	}

	@Test
	void shouldAllowBurstAcrossWindowBoundary() {
		TestClock clock = new TestClock(Instant.now());
		FixedCounterRateLimitAlgorithm algorithm = new FixedCounterRateLimitAlgorithm(5, 5, clock);
		...
		for (int i = 0; i < 5; i++) {
			assertTrue(algorithm.acceptRequest(), "Should allow burst after window expires");
		}
	}
	...
	void windowCountsShouldBeIsolatedPerKey() {
		TestClock clock = new TestClock(Instant.now());
		FixedCounterRateLimitAlgorithm algorithm1 = new FixedCounterRateLimitAlgorithm(3, 10, clock);
		FixedCounterRateLimitAlgorithm algorithm2 = new FixedCounterRateLimitAlgorithm(3, 10, clock);
